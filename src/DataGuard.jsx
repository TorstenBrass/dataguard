...
function MainDashboard({ onGoToSubmitReview }) {
  ...
  const filtered = APP_DB.filter(a => {
          ...
          <div>
            <div style={{display:\"flex\",gap:\"16px\",marginBottom:\"24px\",flexWrap:\"wrap\"}}>
              ...
              <div style={{display:\"flex\",gap:\"8px\",overflowX:\"auto\",paddingBottom:\"4px\"}}>
                {categories.map(c => (
                  <button key={c} onClick={()=>setSelectedCat(c)} style={{background:selectedCat===c?\"#FFF\":\"#121214\",color:selectedCat===c?\"#000\":\"rgba(255,255,255,0.6)\",border:\"1px solid rgba(255,255,255,0.08)\",padding:\"0 20px\",height:\"54px\",borderRadius:\"12px\",fontWeight:\"700\",cursor:\"pointer\",whiteSpace:\"nowrap\",fontFamily:\"'Syne', sans-serif\"}}>
                    {c}
                  </button>
                ))}
              </div>
...
function SubmitReviewScreen({ onBack }) {
  ...
  // Dynamic values calculated by our engine
  ...
  // Automated evaluation score calculator function
  useEffect(() => {
    let baseScore = 100;

    // Deduct penalties for background configurations
    ...
    // Deduct individual data tracking penalties
    dataTypes.forEach(typeId => {
      const match = availableDataTypes.find(d => d.id === typeId);
      if (match) {
        baseScore -= match.penalty;
      }
    });

    // Constrain score boundaries between 0 and 100
    const finalScore = Math.max(0, Math.min(100, baseScore));
    setCalculatedScore(finalScore);

    // Determine algorithmic letter grade matching standard score frameworks
    let letter = \"F\";
      ...
      <div style={{maxWidth:\"800px\",margin:\"0 auto\"}}>
        ...
        {!submitted ? (
          <form onSubmit={handleSubmit} style={{display:\"flex\",flexDirection:\"column\",gap:\"32px\"}}>
            {/* Real-time Assessment Evaluation Panel */}
