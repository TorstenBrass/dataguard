...
const APP_DB = [
  ...
  // ── BATCH 4: NEW ADDITIONS ──────────────────────────────────────────────────
  { id:49, name:"X (Twitter)",              category:"Social Media",            icon:"𝕏", score:18, privacyGrade:"F",  playStoreId:"com.twitter.android",               dataTypes:["Location","Contacts","Browsing History","Device ID","Ad Interactions","Biometrics","IP Address","Search History","DMs Content","Voice Data (Spaces)","Grok AI Training Data"], sellsData:true,  misleadingAds:true,  thirdParties:38, founded:"2006", headquarters:"San Francisco, USA",                       summary:"Rebranded to X under Elon Musk's ownership. Collects extensive behavioral data including browsing history across the web via embedded widgets. User posts are fed into Grok AI training by default. Has faced multiple GDPR enforcement actions from the Irish DPC, and a €120M DSA fine from the EU Commission in 2025 for transparency violations. Shares data with advertising partners and third-party analytics providers.", sources:["EU Commission DSA Fine 2025","Irish DPC Enforcement 2024","EFF Analysis","EPIC.org 2025 Grok Investigation"], communityFlags:2156, communityVerified:true, knownIncidents:["€120M EU DSA fine (2025)","Irish DPC High Court action over data processing (2024)","Part of €3.9B collective social media GDPR fines","200M+ user email leak (2023)"] },
  { id:50, name:"Amazon Shopping",          category:"E-Commerce",              icon:"📦", score:22, privacyGrade:"F",  playStoreId:"com.amazon.mShop.android.shopping", dataTypes:["Precise Location","Contacts","Purchase History","Financial Info","Browsing History","Voice Data (Alexa)","Photos","Camera Access","Search History","Device ID","Biometrics","Wi-Fi Credentials"], sellsData:true,  misleadingAds:true,  thirdParties:41, founded:"1994", headquarters:"Seattle, USA",                              summary:"One of the most data-hungry shopping apps. Collects everything from purchase history to voice recordings, camera feeds, Wi-Fi credentials, and even household device usage. Shares data with advertising partners, third-party sellers, payment processors, and co-branded partners. Received a record €746M GDPR fine for manipulating ad targeting without proper consent.", sources:["Luxembourg CNPD €746M Fine (2021)","Amazon Privacy Notice 2025","Google Play Data Safety","EFF Surveillance Report"], communityFlags:1847, communityVerified:true, knownIncidents:["€746M GDPR fine — largest ever at time of issue (2021)","Ring doorbell data shared with police without warrants","Alexa recordings reviewed by contractors","Employee access to customer order data"] },
  { id:51, name:"Steam",                    category:"Gaming",                  icon:"🎮", score:45, privacyGrade:"D+", playStoreId:"com.valvesoftware.android.steam.community", dataTypes:["Purchase History","Financial Info","Device ID","IP Address","Hardware Config","Gameplay Statistics","Friends List","Chat Messages","Search History","MAC Address","Geolocation"], sellsData:false, misleadingAds:false, thirdParties:12, founded:"2003", headquarters:"Bellevue, USA",                             summary:"Collects detailed hardware configs, gameplay statistics, and purchase history. Shares data with advertising service providers including Google, Facebook, and Doubleclick for targeted ads. Uses cookies, web beacons, and flash cookies for tracking. Gameplay data collected offline and transmitted later. No major breaches confirmed, but broad data collection for a gaming platform.", sources:["Valve Privacy Policy","Apple App Store Data Safety","Common Sense Privacy Report","Steam Subscriber Agreement"], communityFlags:423, communityVerified:true, knownIncidents:["89M alleged records appeared on dark web (2024, Valve denied breach)","Historical Steam Guard vulnerabilities","VAC system data collection concerns"] },
  { id:52, name:"Xbox",                     category:"Gaming",                  icon:"🎯", score:28, privacyGrade:"D",  playStoreId:"com.microsoft.xboxone.smartglass",  dataTypes:["Precise Location","Device ID","IP Address","Voice Data","Chat Messages","Friends List","Gameplay Statistics","Purchase History","Photos","Videos","Achievements","Play Time","Hardware Errors","Content Uploads"], sellsData:false, misleadingAds:false, thirdParties:24, founded:"2001", headquarters:"Redmond, USA",                              summary:"Microsoft's gaming platform collects extensive behavioral and performance data. Shares user data with game publishers including gamertags, activity feeds, and in-game communications. Microsoft augments collected data with information purchased from data brokers. Paid $20M FTC fine for illegally collecting children's data without parental consent. Voice, text, images monitored for community standards enforcement.", sources:["FTC COPPA Settlement 2023","Mozilla Privacy Not Included","Microsoft Privacy Statement","Xbox Support Documentation"], communityFlags:892, communityVerified:true, knownIncidents:["$20M FTC fine for COPPA violations (2023)","38TB data exposed by Microsoft AI researchers (2023)","60K emails stolen via Microsoft services (2023)","250M customer service logs exposed (2019)"] },
...
function FreeDemo({ onUnlock }) {
  ...
  const handleSearch = () => {
    if (usedFree) return;
    const found = APP_DB.find(a => a.name.toLowerCase().includes(query.toLowerCase()));
    if (found) { setDemoResult(found); setUsedFree(true); setNotFound(false); }
    else { setNotFound(true); }
  };

  return (
    <div style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:"20px",padding:"40px 24px",textAlign:"center",maxWidth:"600px",margin:"0 auto"}}>
      <h3 style={{color:"white",fontSize:"20px",marginBottom:"8px"}}>🔍 Try it free — check one app now</h3>
      <p style={{color:"rgba(255,255,255,0.5)",fontSize:"14px",marginBottom:"20px"}}>See what we expose. No signup required.</p>
      {!usedFree ? (
        <div>
          <div style={{display:"flex",gap:"8px",maxWidth:"400px",margin:"0 auto"}}>
            <input value={query} onChange={e=>setQuery(e.target.value)} placeholder="e.g. TikTok, Instagram, Temu..." onKeyDown={e=>e.key==="Enter"&&handleSearch()} style={{flex:1,background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:"12px",color:"white",padding:"13px 16px",fontSize:"15px",outline:"none"}}/>
            <button onClick={handleSearch} style={{background:"#ef5350",color:"white",border:"none",padding:"13px 20px",borderRadius:"12px",fontWeight:"700",cursor:"pointer",flexShrink:0}}>Check</button>
          </div>
          {notFound&&<p style={{color:"rgba(255,255,255,0.4)",fontSize:"13px",marginTop:"10px"}}>App not found — try a different name.</p>}
        </div>
      ) : demoResult ? (
        <div style={{textAlign:"left",background:"rgba(0,0,0,0.3)",borderRadius:"16px",padding:"20px",marginTop:"16px"}}>
          <div style={{display:"flex",alignItems:"center",gap:"12px",marginBottom:"12px"}}>
            <span style={{fontSize:"28px"}}>{demoResult.icon}</span>
            <div style={{flex:1}}>
              <h4 style={{color:"white",margin:"0 0 4px"}}>{demoResult.name}</h4>
              <span style={{color:gradeColor(demoResult.privacyGrade),fontWeight:"700",fontSize:"14px"}}>Grade: {demoResult.privacyGrade}</span>
            </div>
            <ScoreRing score={demoResult.score} size={50}/>
          </div>
          <p style={{color:"rgba(255,255,255,0.7)",fontSize:"13px",lineHeight:"1.5"}}>{demoResult.summary.slice(0,220)}...</p>
          <p style={{color:"#ef5350",fontSize:"13px",fontWeight:"700",marginTop:"12px"}}>⚠️ Shares data with {demoResult.thirdParties} third parties</p>
          <div style={{borderTop:"1px solid rgba(255,255,255,0.1)",marginTop:"16px",paddingTop:"16px",textAlign:"center"}}>
            <p style={{color:"rgba(255,255,255,0.5)",fontSize:"13px",marginBottom:"12px"}}>Want full details + 48 more apps?</p>
            <button onClick={onUnlock} style={{background:"#ef5350",color:"white",border:"none",padding:"12px 24px",borderRadius:"10px",fontWeight:"700",cursor:"pointer",fontSize:"15px"}}>Unlock Full Access — $7</button>
          </div>
        </div>
      ) : null}
    </div>
...
function LandingScreen({onPurchase}) {
  ...
  // ── BATCH 5: AMAZON, STEAM, XBOX ────────────────────────────────────────────
  { id:45, name:"Amazon Shopping", category:"Shopping / E-Commerce", icon:"📦", playStoreId:"com.amazon.mShop.android.shopping", score:36, privacyGrade:"D", dataTypes:["Name","Email","Address","Payment Info","Purchase History","Browsing History","Search History","Voice Data (Alexa)","Device ID","Location","Behavioral Data","Biometrics (Amazon One)"], sellsData:false, misleadingAds:true, thirdParties:22, founded:"1994", headquarters:"Seattle, USA", summary:"Amazon officially states it never sells customer personal data. However a 2025 class action investigation by Keller Rohrback alleges Amazon provided its Ads SDK to thousands of third-party app developers — granting Amazon access to users' mobile phones and user data which Amazon then monetizes through its advertising empire. Amazon collects an extraordinary amount of data across shopping, Alexa voice commands, delivery tracking, and third-party seller interactions. Alexa has been found to retain voice recordings and share them with third parties including contractors. Amazon One biometric palm-print data raised congressional concerns. Combines purchase data with browsing history to build highly detailed consumer profiles for advertising.", sources:["Amazon Privacy Notice December 2025","Keller Rohrback Amazon SDK Class Action Investigation 2025","US Senate Biometric Data Letter 2021","Surfshark Smart Home Privacy Research 2024"], communityFlags:1842, communityVerified:true, knownIncidents:["2025: Class action investigation — Amazon Ads SDK allegedly harvests third-party app user data","Amazon Alexa contractors confirmed listening to private voice recordings","Amazon One palm-print biometric collection raised US Senate concerns","Amazon combines purchase, browsing, and Alexa data into unified ad profiles used across the internet"] },
  { id:46, name:"Steam", category:"Gaming / Digital Distribution", icon:"🎮", playStoreId:"com.valvesoftware.android.steam.community", score:62, privacyGrade:"C", dataTypes:["Email","Username","Payment Info","Purchase History","Game Activity","Playtime","Device Info","IP Address","Chat Messages","Community Posts","Usage Statistics"], sellsData:false, misleadingAds:false, thirdParties:8, founded:"2003", headquarters:"Bellevue, USA (Valve Corporation)", summary:"Steam is better than most gaming platforms on privacy. Valve does not sell personal data to third-party advertisers and removed Google Analytics in 2023 to improve privacy. However it collects extensive gaming behavior data — every game you play, for how long, every achievement — and shares this data with game developers and publishers when you play their games. Valve may transfer your data outside the EU/EEA. Anonymous aggregated data is shared freely with third parties. Valve uses your data to serve targeted recommendations and personalized marketing within Steam. Payment data including credit card details is collected and stored.", sources:["Steam Privacy Policy (Valve)","Common Sense Privacy Report: Steam","CyberPost Steam Privacy Analysis May 2025","TechUnwrapped Steam Privacy Overview"], communityFlags:287, communityVerified:true, knownIncidents:["Game playtime, achievements, and activity shared with all game developers and publishers you play","Payment and credit card data stored by Valve","Data may be transferred outside EU/EEA without always-adequate safeguards","Valve removed Google Analytics 2023 — a positive privacy step but data sharing with developers continues"] },
  { id:47, name:"Xbox", category:"Gaming", icon:"🎯", playStoreId:"com.microsoft.xboxone.smartglass", score:44, privacyGrade:"D", dataTypes:["Name","Email","Age","Gamertag","Game Activity","Achievements","Playtime","Voice Data","Device Info","Location","Behavioral Data","Payment Info","Controller Input Data"], sellsData:false, misleadingAds:true, thirdParties:16, founded:"2001", headquarters:"Redmond, USA (Microsoft)", summary:"Xbox does not sell personal data but Microsoft uses it extensively for advertising across Microsoft properties and shares it with third parties including Facebook and Yahoo for ad targeting. Microsoft collects controller button presses, voice data, and gaming habits. When you play any third-party game on Xbox, your data is automatically shared with that game's publisher — your Gamertag, country, age range, achievements, and playtime. In 2023 the FTC fined Microsoft $20 million for COPPA violations — illegally collecting children's personal data through Xbox without parental consent. Microsoft also sources additional data about you from data brokers and social networks to combine with your Xbox profile.", sources:["Microsoft Xbox Privacy Data Collection Summary October 2025","Mozilla Foundation Xbox Privacy Not Included Review","FTC $20M COPPA Settlement 2023","Xbox Third Party Data Sharing Policy"], communityFlags:634, communityVerified:true, knownIncidents:["2023: FTC fined Microsoft $20M for COPPA violations — illegally collecting children's data on Xbox","Your data automatically shared with all third-party game publishers when you play their games","Microsoft combines Xbox data with information from data brokers and social networks","Shares data with Facebook and Yahoo for cross-platform ad targeting"] },

  // ── BATCH 6: PLAYSTATION ─────────────────────────────────────────────────────
  { id:48, name:"PlayStation App", category:"Gaming", icon:"🎮", playStoreId:"com.playstation.mobile2.prod.android", score:40, privacyGrade:"D", dataTypes:["Name","Email","Date of Birth","PSN ID","Game Activity","Trophies","Playtime","Chat Messages","Friends List","Device Info","IP Address","Payment Info","Behavioral Data","Location"], sellsData:false, misleadingAds:true, thirdParties:14, founded:"1994", headquarters:"San Mateo, USA (Sony Interactive Entertainment)", summary:"PlayStation does not sell personal data but tracks everything you do across its network — every game played, every trophy earned, how long you play, who you chat with, and what content you browse. This data is used to serve targeted ads across third-party websites and apps. Your public profile including activity and trophies is visible by default to all PlayStation Network users and can appear on third-party sites. Sony has the worst data breach history in the gaming industry: 2011 breach exposed 77 million accounts including names, emails, addresses and passwords — one of the largest consumer breaches in history. Sony was breached again in 2014, 2017, and twice in 2023 via the MOVEit ransomware attack and RansomedVC hacker group.", sources:["PlayStation Privacy Policy (SIE)","Common Sense Privacy Report: PlayStation","Huntress Sony Data Breach Analysis 2025","Mozilla Privacy Not Included Review"], communityFlags:1203, communityVerified:true, knownIncidents:["2011: PSN breach exposed 77 million accounts — names, emails, addresses, passwords — one of the largest consumer breaches ever","2011: $171 million in damages, 23 days of network downtime","2014: $15 million class action settlement plus attorney fees","2023: MOVEit ransomware breach exposed employee Social Security Numbers","2023: RansomedVC hacker group claimed 260GB of proprietary Sony data stolen","2025: PlayStation age verification rollout raised new privacy and server security concerns"] },
];
  return (
    <div style={{minHeight:"100vh",background:"#060610",color:"white",fontFamily:"'Syne', sans-serif",overflowX:"hidden"}}>
      <div style={{position:"relative",minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"40px 24px",textAlign:"center",overflow:"hidden"}}>
        ...
        <div style={{position:"relative",zIndex:1,maxWidth:"680px"}}>
          <p style={{color:"#ef5350",fontSize:"12px",fontWeight:"700",letterSpacing:"2px",textTransform:"uppercase",marginBottom:"16px",fontFamily:"'DM Mono', monospace"}}>PRIVACY WATCHDOG</p>
          <h1 style={{margin:"0 0 20px",fontSize:"clamp(32px, 6vw, 60px)",fontWeight:"800",lineHeight:"1.1",color:"white",fontFamily:"'Syne', sans-serif"}}>Find out which apps are<br/><span style={{color:"#ef5350"}}>selling your data</span></h1>
          <p style={{color:"rgba(255,255,255,0.6)",fontSize:"clamp(16px,2vw,19px)",maxWidth:"540px",margin:"0 auto 32px",lineHeight:"1.6"}}>Privacy scores for 52+ apps — built from real data, not marketing claims. See exactly who's harvesting your information and who they're selling it to.</p>
          <div style={{display:"flex",gap:"12px",justifyContent:"center",flexWrap:"wrap",marginBottom:"16px"}}>
            <button onClick={()=>document.getElementById('free-demo').scrollIntoView({behavior:'smooth'})} style={{background:"#ef5350",color:"white",border:"none",padding:"14px 28px",borderRadius:"12px",fontSize:"16px",fontWeight:"700",cursor:"pointer",fontFamily:"'Syne', sans-serif"}}>🔍 Check Any App Free</button>
            <button onClick={onPurchase} style={{background:"rgba(255,255,255,0.08)",color:"white",border:"1px solid rgba(255,255,255,0.15)",padding:"14px 28px",borderRadius:"12px",fontSize:"16px",fontWeight:"700",cursor:"pointer",fontFamily:"'Syne', sans-serif"}}>Get Full Access — $7</button>
          </div>
          <p style={{color:"rgba(255,255,255,0.35)",fontSize:"12px",fontFamily:"'DM Mono', monospace"}}>OPEN SOURCE · ZERO DATA COLLECTED · ONE-TIME PAYMENT</p>
        </div>
      </div>
      <div style={{display:"flex",justifyContent:"center",gap:"40px",flexWrap:"wrap",padding:"32px 24px",borderTop:"1px solid rgba(255,255,255,0.05)",borderBottom:"1px solid rgba(255,255,255,0.05)"}}>
        {[["52+","Apps Analyzed"],["847","Data Brokers Mapped"],["42K+","Community Flags"],["0","Data Points Collected About You"]].map(([num,label])=>(
          <div key={label} style={{textAlign:"center"}}>
            <div style={{color:"#ef5350",fontSize:"28px",fontWeight:"800",fontFamily:"'DM Mono', monospace"}}>{num}</div>
            <div style={{color:"rgba(255,255,255,0.5)",fontSize:"12px",textTransform:"uppercase",letterSpacing:"1px",marginTop:"4px"}}>{label}</div>
          </div>
        ))}
      </div>
      <div id="free-demo" style={{padding:"48px 24px"}}>
        <FreeDemo onUnlock={onPurchase}/>
      </div>
      <div style={{maxWidth:"900px",margin:"0 auto",padding:"0 24px 80px"}}>
        ...
        <div style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:"24px",padding:"40px",display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))",gap:"32px",textAlign:"center",marginBottom:"64px"}}>
          {[["52+","apps in database"],["18,000+","community reports"],["$0","ad revenue"],["100%","open source"],["0","data points collected about you"]].map(([n,l],i)=>(
